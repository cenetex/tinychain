...
class Wallet {
    constructor(address, secret) {
        if (!address || !secret) {
            ...
            this.secret = forge.util.encode64(keypair.privateKey.toString('hex'));
            this.address = forge.util.encode64(keypair.publicKey.toString('hex'));

        }
    }
    sign(message) {
        return forge.util.encode64(ED25519.sign({
            message: message,
            // also accepts `binary` if you want to pass a binary string
            encoding: 'utf8',
            // node.js Buffer, Uint8Array, forge ByteBuffer, binary string
            privateKey: Buffer.from(forge.util.decode64(this.secret), "hex")
        }).toString('hex'));
    }
    verify(message, signature) {
        ...
        return ED25519.verify({
            ...
            signature: signature,
            // node.js Buffer, Uint8Array, forge ByteBuffer, or binary string
            publicKey: Buffer.from(forge.util.decode64(this.address), "hex")
        });
    }
