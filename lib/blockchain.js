...
class Block {
    constructor(timestamp, data) {
        ...
        this.data = encode64(JSON.stringify(data || {}));
        this.previousHash = "0";
        this.nonce = 0;
    ...
    static load(data) {
        ...
        block.data = encode64(JSON.stringify(data.data));
...
class Blockchain{
    ...
    mine(address){
        if(!address) {
            throw new Error('Reward address is undefined.');
        ...
        newBlock.data = encode64(JSON.stringify({
            wallets: wallets,
        ...
        if(test_result === this.difficulty) {
            ...
            console.warn('failed to mine block');
            return null;
        }
        return this.blocks;
    }
